...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        echo "<h1> Category ID: ". $row["category_id"]. "</h1>";
...
$sql = "delete from pictures WHERE product_id=" . $_GET["id"];
echo "<br>";

if (mysqli_query($conn, $sql)) {
    echo "Record from Picture deleted successfully";
} else {
...
if (mysqli_query($conn, $sql)) {
    echo "Record from Product deleted successfully";
} else {
    echo "Error deleting Product";
}

$sql = "delete from categories WHERE id=" .$c_id;
echo "<br>";
if (mysqli_query($conn, $sql)) {
    echo "Record from category deleted successfully";
} else {
