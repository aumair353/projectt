...
<div class="container">
    <div class="row">
        <div class="col-md-12">
            ...
            $p_id;
            $u_id=$_SESSION["id"];
            $sql = "SELECT products.id,title,price,status,picture_file_name from products,pictures" . " where products.id=pictures.product_id && status=0";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                // output data of each row
                while($row = $result->fetch_assoc()) {
                    ...
                    echo "<h5 class='font-weight-bold'><a href='approve.php?id=$p_id'>Approve Item</a>"."</h5>";
                    echo "</div>";

                    echo "<div class='col-md-2'>";
                    // echo $_SESSION['p_id'];
                    echo "<h5 class='font-weight-bold'><a href='reject.php?id=$p_id'>Reject Item</a>"."</h5>";
                    echo "</div>";
                ...
                echo "<h6 class='text-uppercase'>No Product Added By Seller Yet</h6>";
                echo "</div>";
                echo "</div>";
