<?php
ob_start();
session_start();
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Primestore</title>
    <link rel="icon" href="data:image/jpeg;base64,">
    <link rel="stylesheet" href="../bootstrap-4.1.3-dist/css/bootstrap.min.css">
    <link rel="stylesheet" href="bootstrap-4.1.3-dist/css/bootstrap.min.css">
    <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.3.1/css/all.css" integrity="sha384-mzrmE5qonljUremFsqc01SB46JvROS7bZs3IO2EmfFsd15uHvIt+Y8vEf7N7fWAU" crossorigin="anonymous">
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/css/bootstrap.min.css" integrity="sha384-MCw98/SFnGE8fJT3GXwEOngsV7Zt27NXFoaoApmYm81iuXoPkFOJwJ8ERdknLPMO" crossorigin="anonymous">
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/js/bootstrap.min.js" integrity="sha384-ChfqqxuZUCnJSK3+MXmPNIyE6ZbWh2IMqE241rYiqJxyMiZ6OW/JmZQ5stwEULTy" crossorigin="anonymous"></script>
    <link rel="stylesheet" href="style.css">
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.3/umd/popper.min.js"></script>
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.1.3/js/bootstrap.min.js"></script>


</head>
<body>
<div class="container-fluid header_login">
    <div class="container">
        <div class="row">
            <div class="col-md-4">
                <h1 class="text-left"><a style="color: white ; text-decoration: none" href="index.php">Primestore</a></h1>
            </div>
            <div class="col-md-8">
        <nav class="navbar navbar-expand-sm  navbar-dark">
            <ul class="navbar-nav ml-auto">
                <?php

                if(!isset($_GET['id'])){
                    header("location: index.php");
                }

                require_once("connection.php");
                $type;
                //echo "<h1>". $_GET["id"]. "</h1>";
                //echo "Session value:". $_SESSION["id"];
                if(isset($_SESSION['id'])) {
                    $id=$_SESSION["id"];
                    $sql = "SELECT * from users" . " where id= '$id'";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        // output data of each row
                        while ($row = $result->fetch_assoc()) {
                            echo "<li class='nav-item'><h2 style='color: white'>Hey " . $row["name"] . "</h2></li>";
                            echo "<li class=\"nav-item\">";
                            echo "<a class=\"nav-link\" href='login.php'>Log Out</a>";
                            echo "</li>";
                            $type=$row['type'];
                        }
                    } else {
                        echo "0 results";
                    }
                }
                else{
                    echo  "<li class=\"nav-item\">";
                    echo "<a class=\"nav-link\" href=\"login.php\">Login</a>";
                    echo "</li>";
                    echo "<li class=\"nav-item\">";
                    echo "<a class=\"nav-link\" href='create-new-customer.php'>SignUp</a>";
                    echo "</li>";
                }
                ?>
            </ul>
        </nav>
            </div>
        </div>
    </div>
</div>

<div class="container">
    <?php
    require_once ("connection.php");
    $cc_id=$_GET['id'];
    // echo $cc_id;
    ?>
<div class="row">
    <div class="col-md-12">
<?php

        $product_id;
        $pic;
        $p_id;
        $sql = "select * from users,products,categories, pictures where products.category_id = '$cc_id' && categories.id='$cc_id' && products.id= pictures.product_id && users.id=products.user_id";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
        // output data of each row
        while ($row = $result->fetch_assoc()) {
        $c_id=$row['category_id'];
        // echo "<h1>". $row["title"]. "</h1>";
        // echo "<h1>". $row["price"]. "</h1>";
        // echo "<h1>". $row["status"]. "</h1>";
        //echo "<h1>". $row["picture_file_name"]. "</h1>";
        $pic = $row["picture_file_name"];
        // echo "<img src=\"Images/".$pic.".jpg\">";
        echo "<div class='row'>";
            echo "<div class='col-md-4'>";
            echo "<img src=\"Images/" . $pic . ".jpg\" style=\"width: 100%\">";
            echo "</div>";

            echo "<div class='col-md-4'>";
            echo "<h5 class='font-weight-bold'>Product ID: ". $row["product_id"]. "</h5>";
            $product_id=$row['product_id'];
            echo "<h5 class='font-weight-bold'>Title: ". $row["title"]. "</h5>";
            echo "<h5 class='font-weight-bold'>Price: ". $row["price"]. "</h5>";
            echo "<h5 class='font-weight-bold'>Description: ". $row["description"]. "</h5>";
            echo "<h5 class='font-weight-bold'>Date: ". $row["date_added"]. "</h5>";
            echo "<h5 class='font-weight-bold'>Category: ". $row["name"]. "</h5>";
            echo "<h5 class='font-weight-bold'>Brand: ". $row["shop_title"]. "</h5>";

            if(isset($_SESSION['id'])){
                $userid=$_SESSION['id'];
                $sql = "SELECT * from reviews" . " where user_id= '$userid' && product_id='$product_id'";
                $result = $conn->query($sql);
                if ($result->num_rows > 0) {
                    // output data of each row
                    while($row = $result->fetch_assoc()) {
                        //echo "rating: " . $row["rating"]. "<br>";
                    }
                }
                else {

              echo "<div class=\"card\">";
            echo "<div class=\"card-header\"><h5>Review</h5></div>";
        echo "<div class=\"card-body\">";
         echo "<form action='add-review.php' method='post'>";
            echo "<div class='form-check-inline'>";
             echo "<label class='form-check-label'>";
             echo "<input type='radio' class='form-check-input' name='check' value='1'>Poor";
                echo "</label>";
             echo "</div>";

                echo "<div class='form-check-inline'>";
                echo "<label class='form-check-label'>";
                echo "<input type='radio' class='form-check-input' name='check' value='2'>Satisfactory";
                echo "</label>";
                echo "</div>";

                echo "<div class=\"form-check-inline\">";
                echo "<label class=\"form-check-label\">";
                echo "<input type=\"radio\" class=\"form-check-input\" name='check' value='3'>Good";
                echo "</label>";
                echo "</div>";

                echo "<div class=\"form-check-inline\">";
                echo "<label class=\"form-check-label\">";
                echo "<input type=\"radio\" class=\"form-check-input\" name='check' value='4'>Very Good";
                echo "</label>";
                echo "</div>";

                echo "<div class=\"form-check-inline\">";
                echo "<label class=\"form-check-label\">";
                echo "<input type=\"radio\" class=\"form-check-input\" name='check' value='5'>Excellent";
                echo "</label>";
                echo "</div>";

                echo "<input type='text' name='pid' value='$product_id' hidden>";
                echo "<input type='text' name='c_id' value='$c_id' hidden>";
                echo "<div class=\"form-group\">";
                echo "<label for=\"email\">Comment:</label>";
                echo "<textarea type=\"email\" class=\"form-control\" placeholder=\"Enter Comment\" name=\"comment\"></textarea>";
                echo "</div>";

               echo "<button type=\"submit\" name='submit' class=\"btn btn-primary\">Submit</button>";
               if(isset($_POST['submit'])) {
                   header("location:add-review.php");
               }
               }

           echo "</form>";
        echo "</div>";
   echo "</div>";
            }

            echo "</div>";

            $pr_title=$row["title"];
            $pr_pic=$row["picture_file_name"];

            echo "<div class='col-md-4'>";
            echo "<h6 class='font-weight-bold'><a href='insert-to-cart.php?name=$pr_title && pic=$pr_pic '>ADD TO CART</a>"."</h6>";
            echo "<h6 class='font-weight-bold'><a class='text-uppercase' href='cart.php'>VIEw CART</a>"."</h6>";

            $sql = "SELECT * from reviews,users" . " where product_id = '$product_id' && users.id=reviews.user_id";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                // output data of each row
                while($row = $result->fetch_assoc()) {
                    echo "<div class='card bg-secondary text-white'>";
                    echo "<div class='card-header bg-dark text-white'><h5>Reviews</h5></div>";
                    echo "<div class='card-body'>";
                    echo "<h6>Name: ". $row["name"]. "</h6>";
                    echo "<h6>Rating: ". $row["rating"]. "</h6>";
                    echo "<h6>Comment: ". $row["comments"]. "</h6>";
                    echo "<h6>Date: ". $row["date_added"]. "</h6>";
                    echo "</div>";
                    echo "</div>";
                }
            } else {
                echo "<div class='card bg-secondary text-white'>";
                echo "<div class='card-header bg-dark text-white '><h5>Reviews</h5></div>";
                echo "<div class='card-body'>";
                echo "<h6>No Rating</h6>";
                echo "</div>";
                echo "</div>";
            }

            echo "</div>";

            echo "</div>";
        }
        } else {
        echo "Your Shop is Empty";
        }

        ?>

    </div>

</div>
</div>


<?php
include("footer.php");
?>

</body>
</html>