...
<div class="container">
   ...
   if(!$_SESSION["id"]){
       header("location: login.php?message=You Need to Login First");
   }
   else{
       ...
       if ($result->num_rows > 0) {
           ...
           while($row = $result->fetch_assoc()) {
               // echo "<h1>". $row["shop_title"]. " by " . $row["name"]. "</h1>";
               //echo "<h1>" . $row["type"]. "</h1>";
               $type=$row['type'];
           }
       } else {
 ...
 //  echo "Session value:". $_SESSION["id"];
   ...
   ?>
    ...
    <div class="col-md-2 ml-auto">
        <form method="post">
       <button type="submit" name="submit" class="btn btn-dark btn-outline-dark">Add New Product</button>
        </form>
        <?php
           if(isset($_POST["submit"])){
               header("location: new-product.php?id=$id");
               ob_end_flush();
           }
        ?>
    </div>
...
<div class="container">
    <div class="row">
        <div class="col-md-12">
            ...
            $p_id;
            $u_id=$_SESSION["id"];
            $sql = "SELECT products.id,title,price,status,picture_file_name from products,pictures" . " where user_id= '$u_id' && products.id=pictures.product_id";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
            // output data of each row
            while($row = $result->fetch_assoc()) {
               ...
               // echo $pic;
                ...
                echo "<div class='col-md-4'>";
                echo "<h2 class='font-weight-bold'> ID: ". $row["id"]. "</h2>";
                echo "<h2 class='font-weight-bold'> Title: ". $row["title"]. "</h2>";
                 echo "<h2 class='font-weight-bold'> Price: ". $row["price"]. "</h2>";
                 echo "<h2 class='font-weight-bold'> Status: ". $row["status"]. "</h2>";
                echo "</div>";

                echo "<div class='col-md-2'>";
                echo "<h5 class='font-weight-bold'><a href='delete-product.php?id=$p_id'>Delete Item</a>"."</h5>";
                echo "</div>";

                echo "<div class='col-md-2'>";
               // $_SESSION["p_id"]=$p_id;
               // echo $_SESSION['p_id'];
                echo "<h5 class='font-weight-bold'><a href='update-product.php?id=$p_id'>Update Item</a>"."</h5>";
                ...
                echo "<h6 class='text-uppercase'>your shop is empty</h6>";
                echo "</div>";
                echo "</div>";
