           ...
           // echo "Product ID:". $_SESSION["p_id"];
            ...
            if ($result->num_rows > 0) {
                ...
                while($row = $result->fetch_assoc()) {
                    ...
                    $sql = "SELECT * from pictures" . " where product_id= '$p_id'";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        // output data of each row
                        while($row = $result->fetch_assoc()) {
                           // echo "<h1> Pic: ". $row["picture_file_name"]. "</h1>";
                ...
                <div class="card-body">
                    <form action="update-product.php" method="post" enctype="multipart/form-data">
                        <div class="form-group">
                            <input type="text" class="form-control" placeholder="Current ID" name="c_idd" value="<?php echo $p_id ?>" hidden>
                        </div>
                        <div class="form-group">
                            <label>Title Name:</label>
                            <input type="text" class="form-control" placeholder="Update Title Name" name="t_name">
                        </div>
                        <div class="form-group">
                            <label>Price:</label>
                            <input type="text" class="form-control" placeholder="Update Price" name="price">
                        </div>
                        <div class="form-group">
                            <label>Picture:</label>
                            <input type="file" class="form-control" placeholder="Update Picture" name="picture">
                        </div>
...
if(isset($_POST['update'])){
   ...
   $sql= "update products set title = '$t_name', price = '$p_price',status='0' " ."WHERE id = '$pp_id'";
    echo "<br>";
    ...
    echo '<h2> Here is some info about the file uploaded : </h2>';
...
// Temporary names, the file stored at server
// if you check the location, the file is not there. so it exist only for request processing life span
// so lets move the file to some other location

// So lets first check, is that folder already exist, if not, then lets create it
...
// To store file with different name, we need file extension, lets get file extension from the file_name
    ...
    $file_name_at_server = time()."_".$pp_id; // Generate a unique file name for the file, using some way e.g. time, random.
   ...
   $sql= "update pictures set picture_file_name = '$file_name_at_server'" ."WHERE product_id = '$pp_id'";
    echo "<br>";
    ...
    $u_id=$_SESSION["id"];
    header("location: seller_dashboard.php?id=$u_id");
    ob_end_flush();
}
?>
