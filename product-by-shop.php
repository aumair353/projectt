        ...
        <div class="col-md-4 text-uppercase">
           ...
           // echo "Session value:". $_SESSION["id"];
            ...
            if ($result->num_rows > 0) {
                // output data of each row
                while($row = $result->fetch_assoc()) {
                    echo "<h1>All Product Listed By ". $row["shop_title"]. "</h1>";
                    //echo "<h1>" . $row["type"]. "</h1>";
                }
            } else {
                echo "0 results";
...
<div class="container">
    <div class="row">
        <div class="col-md-12">
            ...
            $p_id;
            $u_id=$_SESSION["id"];
            $sql = "SELECT  products.category_id,products.id,title,price,status,picture_file_name from products,pictures" . " where user_id= '$u_id' && products.id=pictures.product_id";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                // output data of each row
                while($row = $result->fetch_assoc()) {
                   ...
                   // echo "<h2 class='font-weight-bold'> Category ID: ". $row["category_id"]. "</h2>";
                    echo "<a href='product-detail.php?id=$c_id'><h3 class='font-weight-bold'>Title: ". $row["title"]. "</h3></a>";
